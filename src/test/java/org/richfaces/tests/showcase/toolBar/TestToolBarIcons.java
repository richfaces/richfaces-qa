 ...
 * @version $Revision$
 */
public class TestToolBarIcons extends AbstractShowcaseTest {
	...
	public void testGroupSeparators() {
		...
		checkNumberOfAllGroupsOrItemsSeparators(NUMBER_OF_GROUP_SEP, lineGroupSep, gridGroupSep, discGroupSep, squareGroupSep);
	}
	...
	public void testItemSeparators() {
		...
		checkNumberOfAllGroupsOrItemsSeparators(NUMBER_OF_ITEMS_SEP, lineItemSeparator, gridItemSep, discItemSep, squareItemSep);
	}
	 ...
	 */
	private void checkNumberOfAllGroupsOrItemsSeparators( int numberOfSeparators, JQueryLocator whichLineSep, JQueryLocator whichGridSep,
			JQueryLocator whichDicsSep, JQueryLocator whichSquareSep) {
		
		if( numberOfSeparators != 0) guardXhr(selenium).click(whichLineSep);
		...
		if( numberOfSeparators != 0) guardXhr(selenium).click(whichGridSep);
		...
		if( numberOfSeparators != 0) guardXhr(selenium).click(whichDicsSep);
		...
		if( numberOfSeparators != 0) guardXhr(selenium).click(whichSquareSep);
