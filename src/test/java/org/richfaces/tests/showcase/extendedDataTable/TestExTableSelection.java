...
public class TestExTableSelection extends AbstractExtendedTableTest {
	...
	private void checkRowAndSelectedRowInfo( JQueryLocator row ) {
		...
		assertEquals(actualCarInTheRow, actualCarInTheSelectedCarInfo, "The car from selected row can not differ from the " +
				"car in the selected rows info");
		
	}
	
	/**
	 * Checks whether there is class attribute of particular row, and when it is there retrieves it, returns null otherwise
	 * @param row
	 * @return
	 */
