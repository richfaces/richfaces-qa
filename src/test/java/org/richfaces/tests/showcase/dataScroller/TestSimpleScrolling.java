...
public class TestSimpleScrolling extends AbstractDataScrollerTest {
	...
	public void testFirstPageButton() {
		...
		checkFirstOrLastPageButton(firstPageButton, firstPageButtonDis, lastPageButton, 
				lastPageButtonDis, fastPrevButtonDis, previousButtonDis);
	}
	
	@Test
	public void testLastPageButton() {
		
		checkFirstOrLastPageButton(lastPageButton, lastPageButtonDis, firstPageButton, 
				firstPageButtonDis, fastNextButtonDis, nextButtonDis);
	}
	...
	private void checkNumberOfPagesButtons( int numberOfPage ) {
		...
		JQueryLocator checkingButton = jq("a[class*='" + CLASS_OF_INACTIVE_BUTTON_WITH_NUMBER + "']:contains('" + numberOfPage +"'):first");
	 ...
	 * 
	 * @param checkingButton the button which functionality is checked
	 * @param checkingButtonDis disabled checked button
	 * @param otherButton the other button according to checkingButton, for example when checking button is first page button
	 * 			then otherButton should be last page button, and vice versa
	 * @param otherButtonDis disabled other button
	 * @param fastButtonOfCheckingDis disabled fast button which is between the checking button
	 * @param previousOrNextButtonOfCheckingDis disabled previous or next page button  
	 */
	private void checkFirstOrLastPageButton( JQueryLocator checkingButton, 
			JQueryLocator checkingButtonDis, JQueryLocator otherButton, 
			JQueryLocator otherButtonDis, 
			JQueryLocator fastButtonOfCheckingDis, 
		...
		assertTrue(selenium.isElementPresent( checkingButtonDis ), 
				"The " + checkingButtonDis.getRawLocator() + " button should be disabled");
		
		assertTrue(selenium.isElementPresent( fastButtonOfCheckingDis ), 
		"The " + fastButtonOfCheckingDis.getRawLocator() + " page button should be disabled");
		
		assertTrue(selenium.isElementPresent( previousOrNextButtonOfCheckingDis ), 
		"The " + previousOrNextButtonOfCheckingDis + " button should be disabled");
	}
	
	/**
	 * Checks functionality of two buttons, one is moving forward and second backward
	 * 
	 * @param button1 either forward or backward moving button
	 * @param button2 either forward or backward moving button
	 * @param step the number of pages we are moving by pressing of the button
	 */
	private void checkButton( JQueryLocator button1, JQueryLocator button2, int step) {
		...
		if( button1.equals(nextButton) || button1.equals(fastNextButton) ) {
			assertEquals(numberOfPageAtBeginning, numberOfPageAfterClickOnButton1 - 1, "Next button or fast next button does not work");
		} else {
			assertEquals(numberOfPageAtBeginning, numberOfPageAfterClickOnButton1 + 1, "Previous button or fase previous button does not work");
		}
		...
		if( button2.equals( previousButton) || button2.equals(fastPrevButton) ) {
			assertEquals(currentNumberOfPage, numberOfPageAtBeginning, "Previous button or fase previous button does not work");
		} else {
