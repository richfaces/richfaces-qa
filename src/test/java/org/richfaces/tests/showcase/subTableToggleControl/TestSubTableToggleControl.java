...
public class TestSubTableToggleControl extends AbstractDataIterationWithCars {
	...
	/**
	 *  Checks whether there is more than 0 rows with some data, 
	 *  also checks whether the counting of rows works
	 *  
	...
	private void checksCountingOfRows(JQueryLocator toggler, JQueryLocator bodyOfTable) {
		...
		String expectedNumberOfTrs = 
			selenium.getText(bodyOfTable.getChild(jq("tr:visible:last"))).trim();
	...
	private void checkTogglersFunctionality( JQueryLocator toggler, JQueryLocator subtable ) {
		...
		assertEquals(isSubTableDisplayedAfterFirstClick, !isSubTableDisplayedAfterSecondClick, "The subtable should be displayed, or hidden, " +
