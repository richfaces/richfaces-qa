 ...
 */
public class MessagesComponentImpl implements Iterable<MessageComponent> {

    @Root
    private WebElement root;
    //
    @FindBy(xpath= "./span")
    private List<MessagesMessageComponentImpl> messages;
    //
    private WebDriver driver = GrapheneContext.getProxy();

    public MessageComponent getMessage(int index) {
        return messages.get(index);
    }

    public List<MessageComponent> getMessagesForInput(String inputID) {
        List<MessageComponent> result = Lists.newArrayList();
        for (MessageComponent component : this) {
    ...
    public Iterator<MessageComponent> iterator() {
        final Iterator<MessagesMessageComponentImpl> iterator = messages.iterator();
