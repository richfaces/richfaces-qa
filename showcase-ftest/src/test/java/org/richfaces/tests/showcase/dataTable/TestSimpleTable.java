 ...
 * @version $Revision$
 */
public class TestSimpleTable extends AbstractShowcaseTest {
	...
	public void testTotalOfSanJoseTransports() {
		...
		assertEquals(firstValueOfTransportExpenses, 
				FIRST_VALUE_OF_TRANSPORT, "The first value of transport should be different");
		
		String secondValueOfTransportExpenses = selenium.getText(secondValueOfTransport);
		assertEquals(secondValueOfTransportExpenses, 
				SECOND_VALUE_OF_TRANSPORT, "The second value of transport should be different");
		
		String subTotalOfTransports = selenium.getText(subTotalOfTransportsForSanJose);
		assertEquals(subTotalOfTransports, 
				SUBTOTALS_OF_TRANSPORT_FOR_SAN_JOSE, "The subtotal od transports should be different");
		...
		assertEquals(actualSubtotal, removeDollarAndMakeInt(subTotalOfTransports),
				"Two values are added incorrectly, they are not equal to actual" +
				"counting");
		
		assertEquals(actualSubtotal, removeDollarAndMakeInt
				(SUBTOTALS_OF_TRANSPORT_FOR_SAN_JOSE), "Two values are added incorrectly," +
				"they are not equal to expected counting");
	}
