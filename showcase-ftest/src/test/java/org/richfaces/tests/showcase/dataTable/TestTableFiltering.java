...
public class TestTableFiltering extends AbstractDataIterationWithCars {
	...
	JQueryLocator vendorSelect = jq("select");
	JQueryLocator mileageInput= jq("input[type=text]:first");
	JQueryLocator vinInput = jq("input[type=text]:last");
	JQueryLocator errorMessageLocator = jq("span[class=rf-msgs-sum]");
	JQueryLocator firstRow = jq("tbody[class=rf-dt-b] tr:first");
	JQueryLocator tBody = jq("tbody[class=rf-dt-b]");
	...
	public void testNoFilters() {
		...
		//this solution I choose to achieve that it does not matter in which order are tests running
		//because this state can be inherited from other tests and therefore there is no request
		try {
	...
	public void testVinFilter() {
		...
		assertTrue(checkWhetherAllRowsHaveCorrectValue("vin", VIN_FILTER), "all vins of the car should contain " + VIN_FILTER);
	...
	public void testErrorMessage() {
		...
		assertTrue(actualErrorMessage.contains(ERROR_MESSAGE), "There should be error message, since there is string in the mileage input!");
	...
	public void testMileageVinFilter() {
		...
		assertTrue(checkWhetherAllRowsHaveCorrectMileageAndVin(MILEAGE_FILTER, VIN_FILTER), "There should be only rows with mileage" +
				" less or equal to " + MILEAGE_FILTER + " and vin which contains " + VIN_FILTER);
	...
	public void testNoRowsSatisfyConditions() {
		...
		boolean nothingFoundPresent = selenium.isElementPresent(tBody.getChild(jq("tr > td:contains('Nothing found')")));
	 ...
	 *********************************************************************************************************************/
	/**
	 * Check that all rows in the table have a price, which is correct according to the filter.
	 * So whether the price is lower or equal than value in the filterValue parameter.
	 * 
	 * @param filterValue which should be bigger or equal to all price values in the table
	...
	private boolean checkWhetherAllRowsHaveCorrectMileageAndVin( String mileagefilter, String vinFilter) {
		...
		for( Iterator<JQueryLocator> i = trs.iterator(); i.hasNext(); ) {
			...
			boolean result = checkValueInTheRow(i.next(), mileagefilter, "mileage");
