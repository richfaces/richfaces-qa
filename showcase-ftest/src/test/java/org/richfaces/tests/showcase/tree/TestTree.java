...
public class TestTree extends AbstractTreeTest {
	...
	private JQueryLocator countryNodeLabel = jq(FIRST_LVL_NODE
			+ " > div > span > span.rf-trn-lbl");
	...
	public void testCountAllCollapsedAndExpandAllAndCountAllExpanded() {
		...
		assertFalse(selenium.isElementPresent(allColapsedHnd),
				"All nodes should be now expanded");
		...
		assertEquals(countOfAllCollapsed, countOfAllExpanded,
				"The number of all collapsed should be equals to the number of all expanded "
	...
	public void testSelectCountry() {
		...
		for (int i = 0; i < countOfCountries; i++) {
			...
			JQueryLocator labelFromTree = jq(FIRST_LVL_NODE + ":eq(" + i
					+ ") > " + LABEL_NODE );
			...
			assertEquals(
					labelFromTreeString,
					labelFromCurrentSelectionPanel,
	...
	public void testSelectCompany() {
		...
		for (int j = 0; j < countOfCountries; j++) {
			...
			JQueryLocator companyNodeLabel = jq(FIRST_LVL_NODE + ":eq(" + j + ") >"
					+ SECOND_LVL_NODE + " > " + LABEL_NODE);
			...
			for (int i = 0; i < countOfCompanies; i++) {
				...
				companyNodeLabel = jq(FIRST_LVL_NODE + ":eq(" + j + ") >" + SECOND_LVL_NODE
						+ ":eq(" + i + ")" + " > " + LABEL_NODE );

				String companyNodeLabelStringFromTree = selenium
						.getText(companyNodeLabel);
				...
				assertEquals(
						companyNodeLabelStringFromTree,
						companyLabelCurrentSelectionPanel,
	...
	public void testSelectCD() {
		...
		for ( int j = 0; j < countOfCountries; j++ ) {
			...
			String country = selenium.getText( jq( FIRST_LVL_NODE + ":eq(" + j + ") >"
					+ LABEL_NODE) );
			
			JQueryLocator companyNodeExpandSign = jq( FIRST_LVL_NODE + ":eq(" + j + ") >"
					+ SECOND_LVL_NODE + " > " + EXPAND_SIGN );
			...
			for (int i = 0; i < countOfCompanies; i++) {
				...
				String company = selenium.getText( jq(FIRST_LVL_NODE + ":eq(" + j + ") > "
						+ SECOND_LVL_NODE + ":eq(" + i + ") > " + LABEL_NODE) );
				
				companyNodeExpandSign = jq( FIRST_LVL_NODE + ":eq(" + j + ") > "
						+ SECOND_LVL_NODE + ":eq(" + i + ") > " + EXPAND_SIGN );
				
				selenium.click( companyNodeExpandSign );
				
				JQueryLocator cd = jq( FIRST_LVL_NODE + ":eq(" + j + ") > "
						+ SECOND_LVL_NODE + ":eq(" + i + ") > " +  LEAF + " > " + LABEL_NODE);
				
				int countOfCD = selenium.getCount(cd);
				
				for( int k = 0; k < countOfCD; k++ ) {
					
					cd = jq( FIRST_LVL_NODE + ":eq(" + j + ") > "
							+ SECOND_LVL_NODE + ":eq(" + i + ") > " +  LEAF + ":eq(" + k 
							+ ") > " + LABEL_NODE);
					...
					assertEquals( cdFromTree, cdFromCurrentSelection, "The information about cd should be the same in the tree and in the " +
	...
	public void testRememberExpandedNodeWhenParentIsCollapsed() {
		...
		JQueryLocator firstCountryFirstCompanyExpSign = jq ( FIRST_LVL_NODE + ":eq(0) > " + SECOND_LVL_NODE + ":eq(0) > " + EXPAND_SIGN );
		
		
		selenium.click(firstCountryFirstCompanyExpSign);
		
		JQueryLocator firstCountryFirstCompanyFirstCD = 
			jq( FIRST_LVL_NODE + ":eq(0) > " + SECOND_LVL_NODE + ":eq(0) > " + LEAF + ":eq(0) > " + LABEL_NODE);
		...
		assertEquals( cdBeforeCollapsing, cdAfterCollapsing, "The selected cd should be remained when the branch was closed!");
		
		selenium.click(firstCountryExpandSign);
		
		JQueryLocator firstCountryFirstCompanyCollapseSign = jq(FIRST_LVL_NODE + ":eq(0) > " + SECOND_LVL_NODE + ":eq(0) > " + COLLAPSE_SIGN );
		assertTrue( selenium.isElementPresent( firstCountryFirstCompanyCollapseSign), "The first country first company node should remain expanded " +
				"when the first country was collapsed!" );
	}
	 ...
	 */
	private void collapseOrExpandAllNodes(int numberOfNodes,
			JQueryLocator collapseOrExpand) {
	...
	/**
	 * Gets the name from the panel where the current selection from tree is
	 * shown
	 * 
	 * @return the name from the panel where the current selection from tree is
	 *         shown
	 */
	...
	private CD retrieveCDFromCurrentSelection() {
		...
		for( int i = 0; i < 5; i++) {
			...
			case 0: cd.setCountry( partsOfRow[1].trim() ); break;
			case 1: cd.setCompany( partsOfRow[1].trim() ); break;
			case 2: cd.setArtist( partsOfRow[1].trim() ); break;
			case 3: cd.setPrice( partsOfRow[1].trim() ); break;
			case 4: cd.setYear( partsOfRow[1].trim() ); break;
			default: fail("Error when proccessing of current selection panel");
			}
	...
	protected class CD {
		...
		public CD(String country, String company, String artist, String name,
		...
		public String toString() {
			return "CD [country=" + country + ", company=" + company
					+ ", artist=" + artist + ", name=" + name + ", price="
					+ price + ", year=" + year + "]";
		}
