 ...
 * @version $Revision$
 */
public class AbstractDragAndDropTest extends AbstractGrapheneTest {
    ...
    protected void dragAndDropAllSourcesToCorrectTarget(boolean testAlsoIndicator) {

        for (int i = NUMBER_OF_DRAGGABLE_SOURCES - 1; i >= 0; i--) {

            for (int j = 0; j < NUMBER_OF_DRAGGABLE_TARGETS; j++) {

                int numberOfCurrentDraggableObjectsBefore = selenium.getCount(dragableObject);

                JQueryLocator particulardDragableObject = jq(dragableObject.format(":eq(" + i + ")").getRawLocator());

                Drag drag = new Drag(particulardDragableObject, targets.format(j));
                ...
                int numberOfCurrentDraggableObjectsAfter = selenium.getCount(dragableObject);
    ...
    private void checkingOfIndicator() {
        ...
        assertTrue(selenium.isElementPresent(indicator), "The indicator should be present, when the object is dragged!");

        assertEquals(selenium.getText(indicator), TEXT_OF_INDICATOR, "The indicator text is not correct");
    }
    ...
    private void checkingThePositionOfObjectsAfterDragging() {
        ...
        assertEquals(numberOfCurrentDraggableObjectsAfter, 0, "All draggable objects should be draged to targets!");

        assertEquals(selenium.getCount(phpTableRow), NUMBER_OF_PHP_FRAMEWORKS,
            "The number of dragged php frameworks is wrong!");
        assertEquals(selenium.getCount(dotNetTableRow), NUMBER_OF_DOTNET_FRAMEWORKS,
