...
|Most of tests can run when browser window is not focused, i.e. when it is not top-level window. However, tests using focus and blur browser events require that the browser window is active. Disable all programs that have pop-up windows (e.g. Windows update) while running tests. Another workaround is to use Xvfb, start your tests using `xvfb-run mvn clean verify...`
