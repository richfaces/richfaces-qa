...
public abstract class AbstractRichMessageTest extends AbstractAjocadoTest {
	...
	private static Logger logger = LoggerFactory
			.getLogger(AbstractRichMessageTest.class);
	...
	/**
	 * Because of message and messages have many attributes very similar, this
	 * method helps test method distinguish between metamer page for
	 * rich:message and rich:messages (there are different element IDs per page
	 * and for messages there are messages container rendered conditionally)
	 * 
	...
	public void testHtmlAttribute(ElementLocator<?> element,
			MessageAttributes attribute, String value) {
	...
	protected void testFireEvent(Event event, ElementLocator<?> element) {
		...
		waitGui.failWith(
				"Attribute on" + event.getEventName()
						+ " does not work correctly").until(
	...
	/**
	 * Set wrong values into appropriate inputs and generate validation messages
	 * by submitting form.
	 * 
	 * There are 2 possible ways how to submit: by h:commandButton or by
	 * a4j:commandButton. Switch between them is done by 'byAjax' param
	 * 
	 * @param Boolean
	 *            <b>byAjax</b> - use to choose submit button type used to
	 *            submit form
	 */
	...
	protected void waitForAttribute(MessageAttributes attr) {
		waitGui.until(attributeEquals.locator(
				getTestElemLocator().getAttribute(
						new Attribute(attr.toString()))).text(attr.toString()));
	}

	// ==================== test methods ====================
	/**
	 * ajaxRendered attribute change behavior: messages are displayed after
	 * action performed by a4j:button (not only by h:command*)
	 */
	public void testAjaxRendered() {
		...
		generateValidationMessages(true);
		waitGui.until(elementPresent.locator(jq(getTestElemLocator()
				.getRawLocator() + ":empty")));
	}
	...
	public void testRendered() {
		...
		generateValidationMessages(false);
		waitGui.until(elementNotPresent.locator(jq(getTestElemLocator()
				.getRawLocator())));
	}
	...
	public void testShowSummary() {
		...
		generateValidationMessages(false);
		waitGui.until(elementNotPresent.locator(jq(getSummaryElemLocator()
				.getRawLocator())));
	}
	...
	public void testShowDetail() {
		...
		generateValidationMessages(false);
		waitGui.until(elementNotPresent.locator(jq(getDetailElemLocator()
				.getRawLocator())));
	}
