...
public abstract class Arquillian2 implements IHookable {
...
//        System.out.println(Thread.currentThread().getName() + ", before suite");
...
//        System.out.println(Thread.currentThread().getName() + ", run");
    ...
    @DataProvider(name = Arquillian2.ARQUILLIAN_DATA_PROVIDER)
    ...
    private void verifyTestRunnerAdaptorHasBeenSet() {
        if (deployableTest.get() == null) {
            try {
                // ### OVERRIDE ###
                // the before suite is only called in main thread, we have to call it manually
                arquillianBeforeSuite();
            } catch (Exception ex) {
                Logger.getLogger(Arquillian2.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
