 ...
 */
@ManagedBean(name = "a4jMediaOutputBeanConversationScoped")
@ConversationScoped
public class A4JMediaOutputBeanCScoped implements Serializable {
    ...
    private void createContentFromResource(OutputStream out, Object data, String resourceName) throws IOException {
        InputStream in = null;
        try {
            in = A4JMediaOutputBeanCScoped.class.getClassLoader().getResourceAsStream(resourceName);
            copy(in, out);
        } finally {
            if (in != null) {
                in.close();
