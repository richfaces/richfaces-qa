 ...
 */
@ManagedBean(name = "a4jMediaOutputBeanViewScoped")
@ViewScoped
public class A4JMediaOutputBeanVScoped implements Serializable {
    ...
    private void createContentFromResource(OutputStream out, Object data, String resourceName) throws IOException {
        InputStream in = null;
        try {
            in = A4JMediaOutputBeanVScoped.class.getClassLoader().getResourceAsStream(resourceName);
            copy(in, out);
        } finally {
            if (in != null) {
                in.close();
